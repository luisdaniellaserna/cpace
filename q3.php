...
<head>
  <meta charset="utf-8">
  <title>Management Services Question Config</title>
  ...
  <table id="table1" class="table">
    ...
    <tbody>
      ...
      if (!empty($searchQuery)) {
          $sql = "SELECT * FROM mng WHERE question LIKE '%$searchQuery%'";
      } else {
          $sql = "SELECT * FROM mng";
      }
      ...
      while ($row = $result->fetch_assoc()) {
          ...
          echo "<td>
                <a href='/CPA/q3Edit.php?id={$row['mng_id']}'>
                  <button class='edit-button'>Edit</button>
                </a>
                <a href='/CPA/q3Delete.php?id={$row['mng_id']}'>
